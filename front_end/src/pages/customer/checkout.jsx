...
const Checkout = () => {
    ...
    }, [shop.cart, total, subtotal, setTotal, setSubtotal])
    ...
    const validateForm = () => {
        if (customerName === '' || customerAddress === '' || customerEmail === '' || customerPhone === '') {
            toast.error('You need to complete all the information')
            return false;
    ...
    const handlePlaceOrder = async () => {
        ...
        const resBill = await postCreateBill(customerName, customerAddress, customerPhone, customerEmail, total, subtotal, billCode, createdAt)
        if (resBill && resBill.code === '200') {
            for (const item of listProduct) {
                const newQuantity = item.quantity - item.qty
                await postCreateBillProducts(resBill.data.id, item.id, item.qty)
                await postUpdateQuantityProductAfterPurchase(newQuantity, item.id)
            }
        }
                    ...
                    <form>
                        ...
                        <div className={`mb-10`}>
                            <div className={`flex flex-col mb-4`}>
                                <label className={`text-sm capitalize `}>Your name<span
                                    className={`text-dangerColor-default_2`}>*</span></label>
                                <input onChange={(e) => setCustomerName(e.target.value)}
                                       className={`py-4 pl-6 text-sm text-darkColor rounded-lg border-borderColor focus:border-blackColor hover:border-blackColor mt-2`}
                                       type={`text`}/>
                            </div>
                            <div className={`flex flex-col mb-4`}>
                                <label className={`text-sm capitalize `}>Your address<span
                                    className={`text-dangerColor-default_2`}>*</span></label>
                                <input onChange={(e) => setCustomerAddress(e.target.value)}
                                       className={`py-4 pl-6 mt-2 text-sm text-darkColor rounded-lg border-borderColor focus:border-blackColor hover:border-blackColor`}
                                       type={`text`}/>
                            </div>
                            <div className={`flex flex-col mb-4`}>
                                <label className={`text-sm capitalize `}>Your phone <span
                                    className={`text-dangerColor-default_2`}>*</span></label>
                            ...
                            <div className={`flex flex-col mb-4`}>
                                <label className={`text-sm capitalize `}>Your email<span
                                    className={`text-dangerColor-default_2`}>*</span></label>
                        ...
                        <div className={`mt-4`}>
                            <textarea placeholder={`Notes about your order, e.g. special notes for delivery.`}
                                      className={`text-sm resize-none w-full rounded-xl p-4 border-borderColor focus:border-blackColor hover:border-blackColor`}
                                      cols={12} rows={6}></textarea>
                        </div>
                    </form>
                </div>
                        ...
                        <div className={`w-full`}>
                            <table className={`border-collapse w-full`}>
                                    ...
                                    listProduct.map((item, index) => {
                                        return <tr key={index}
                                            ...
                                            <th className={`text-[18px] font-light text-left break-words align-top py-4 w-10/12`}>
                                                <div>
                                                    <h3 className={`flex items-center mb-1 uppercase text-lg font-medium`}>{item.productName}
                                                        <IoCloseSharp className={`mx-2 text-sm`}/> {item.qty}</h3>
                                ...
                                <tr className={`cart_subtotal table-row`}>
                                    <th className={`text-[18px] font-medium text-left break-all align-top py-4`}>Total
                                        ({total})
                                    </th>
                                    <td className={`table-cell text-right clear-both py-4 align-top`}>
                                        <strong>
                                            <span className={`text-[26px] font-medium text-lime-600`}>${subtotal}</span>
                                        </strong>
                                    </td>
                                </tr>
                                </tbody>
                            ...
                            <div className={`mb-5`}>
                                <div className="flex border-b-[1px] border-borderColor pb-4">
                                    <div className="flex items-center h-5">
                                        <input id="helper-radio" name={`default-radio`}
                                               aria-describedby="helper-radio-text" type="radio" value=""
                                               className="w-4 h-4 text-dangerColor-default_2 bg-gray-100 border-gray-300 focus:ring-dangerColor-default_2 dark:focus:ring-dangerColor-default_2 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
                                    </div>
                                    <div className="ml-2 text-sm">
                                        <label htmlFor="helper-radio"
                                               className="font-medium text-gray-900 dark:text-gray-300">Direct bank
                                            transfer</label>
                                        <p id="helper-radio-text"
                                           className="pt-2 text-xs font-normal text-gray-500 dark:text-gray-300">Make
                                ...
                                <div className="flex border-b-[1px] border-borderColor py-4">
                                    ...
                                    <div className="ml-2 text-sm">
                                        <label htmlFor="helper-radio"
                                               className="font-medium text-gray-900 dark:text-gray-300">Check
                                            payments</label>
                                        <p id="helper-radio-text"
                                           className="pt-2 text-xs font-normal text-gray-500 dark:text-gray-300">Please
                                ...
                                <div className="flex border-b-[1px] border-borderColor py-4">
                                    ...
                                    <div className="ml-2 text-sm">
                                        <label htmlFor="helper-radio"
                                               className="font-medium text-gray-900 dark:text-gray-300">Cash on
                                            delivery</label>
                                        <p id="helper-radio-text"
                                           className="pt-2 text-xs font-normal text-gray-500 dark:text-gray-300">Pay
                                            with cash upon delivery.</p>
                                    </div>
                                </div>
                            </div>
